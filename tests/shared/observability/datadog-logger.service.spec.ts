...
describe('DatadogLoggerService', () => {
  ...
  it('extracts the event field from a structured object message', () => {
    logger.error({ event: 'execution.failed', extra: 'data' }, undefined, 'ErrorContext')
    const entry = lastEntry()
    expect(entry.level).toBe('error')
    expect(entry.message).toBe('execution.failed')
  ...
  it('falls back to defaults when DD_SERVICE/DD_ENV/DD_VERSION are unset', () => {
    logger.log('defaults')
    const entry = lastEntry()
    expect(entry.service).toBe('execution-service')
